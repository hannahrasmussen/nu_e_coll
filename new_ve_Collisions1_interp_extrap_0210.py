...
def f_first_last(f, p4_array, boxsize):
    ...
    p_array = np.arange(len(f))*boxsize

    if j<len(f): #these conditions prevent the code from calling an index of f out of f's bounds
        f_first = ip.interp_log(p4_array[0], p_array, f)
    else:
        f_first = ip.log_linear_extrap(p4_array[0],np.array([(len(f)-2)*boxsize,(len(f)-1)*boxsize]),np.array([f[-2],f[-1]]))
#        f_first = log_linear_extrap(p4_array[0],np.array([0,boxsize]),np.array([f[0],f[1]]))
    if k<len(f)-1:
        f_last = ip.interp_log(p4_array[-1], p_array, f)
    else:
        f_last = ip.log_linear_extrap(p4_array[-1],np.array([(len(f)-2)*boxsize,(len(f)-1)*boxsize]),np.array([f[-2],f[-1]]))
...
def B1_11(p1,E2,T,f,boxsize): #p1 is momentum of incoming neutrino, E2 is energy of incoming electron, T is temp, f is array of f values
    ...
    a = .511
    B = E2
    pEA = max(int((p1+E2-a)/boxsize),int((p1+E2-a)/boxsize+1e-9))
    pEB = max(int((p1+E2-B)/boxsize),int((p1+E2-B)/boxsize+1e-9))
    len_p4 = pEA - pEB + 2
    p4_array = np.zeros(len_p4)

    for i in range(len_p4-2):
        p4_array[i+1] = (pEB+i+1)*boxsize  #array of momenta of neutrinos
    p4_array[0] = p1 + E2 - B 
    p4_array[-1] = p1 + E2 - a    
...
#    q3_array = (E3_array**2-.511**2)**(1/2)
    ...
    for i in range (len_p4-2):
        if pEB+i+1 >= len(f):
            f_holder = ip.log_linear_extrap(p4_array[i+1],np.array([(len(f)-2)*boxsize,(len(f)-1)*boxsize]),np.array([f[-2],f[-1]]))
            if f_holder < 1e-45:
                break
        else:
            f_holder = f[pEB+i+1] 
...
def B7_11(p1,E2,T,f,boxsize): 
    ...
    q3_array = make_q_array(E3_array)
#    q3_array = (E3_array**2-.511**2)**(1/2)
    #q3_array[-1] = 0  I don't know why this was here!
    Fplus_array = np.zeros(len(E3_array))
    Fminus_array = np.zeros(len(E3_array))
    M14_array = np.zeros(len(E3_array))
    M14_array[0] = M14(p1,E2,q2,E3_array[0],q3_array[0])
    ...
    for i in range(len_p4-2):
        ...
        #print(i, len(p4_array), p4_array[i+1]/boxsize, f_holder, f[int(np.round(p4_array[i+1]/boxsize))])
...
def B1_21(p1,E3,T,f,boxsize): 
    ...
    pEA = max(int((p1+a-E3)/boxsize),int((p1+a-E3)/boxsize+1e-9))
    pEB = max(int((p1+B-E3)/boxsize),int((p1+B-E3)/boxsize+1e-9))
    len_p4 = pEB - pEA + 2
    p4_array = np.zeros(len_p4)

    for i in range(len_p4-2):
        p4_array[i+1] = (pEA+i+1)*boxsize  #array of momenta of neutrinos
    p4_array[0] = p1 + a - E3 
    p4_array[-1] = p1 + B - E3
...
#                +A5_11(p1,T,f,boxsize)+A6_11(p1,T,f,boxsize)+A7_11(p1,T,f,boxsize)+A8_11(p1,T,f,boxsize))
    integral21 = A1_21(p1,T,f,boxsize) + A2_21(p1,T,f,boxsize) + A3_21(p1,T,f,boxsize) + A4_21(p1,T,f,boxsize) + A5_21(p1,T,f,boxsize) + A6_21(p1,T,f,boxsize) + A7_21(p1,T,f,boxsize) + A8_21(p1,T,f,boxsize) + A9_21(p1,T,f,boxsize) + A10_21(p1,T,f,boxsize)
...
def R11R22(p1,T,f,boxsize):
    coefficient = 1/(2**4*(2*np.pi)**3*p1**2)
    integral11 = A1_11(p1,T,f,boxsize) + A2_11(p1,T,f,boxsize) + A3_11(p1,T,f,boxsize) + A4_11(p1,T,f,boxsize) + A5_11(p1,T,f,boxsize) + A6_11(p1,T,f,boxsize) + A7_11(p1,T,f,boxsize) + A8_11(p1,T,f,boxsize)
    integral22 = A1_22(p1,T,f,boxsize) + A2_22(p1,T,f,boxsize) + A3_22(p1,T,f,boxsize) + A4_22(p1,T,f,boxsize) + A5_22(p1,T,f,boxsize) + A6_22(p1,T,f,boxsize) + A7_22(p1,T,f,boxsize) + A8_22(p1,T,f,boxsize) + A9_22(p1,T,f,boxsize) + A10_22(p1,T,f,boxsize) + A11_22(p1,T,f,boxsize)
...
def R11R23(p1,T,f,boxsize):
    coefficient = 1/(2**4*(2*np.pi)**3*p1**2)
    integral11 = A1_11(p1,T,f,boxsize) + A2_11(p1,T,f,boxsize) + A3_11(p1,T,f,boxsize) + A4_11(p1,T,f,boxsize) + A5_11(p1,T,f,boxsize) + A6_11(p1,T,f,boxsize) + A7_11(p1,T,f,boxsize) + A8_11(p1,T,f,boxsize)
    integral23 = A1_23(p1,T,f,boxsize) + A2_23(p1,T,f,boxsize) + A3_23(p1,T,f,boxsize) + A4_23(p1,T,f,boxsize) + A5_23(p1,T,f,boxsize) + A6_23(p1,T,f,boxsize) + A7_23(p1,T,f,boxsize) + A8_23(p1,T,f,boxsize) + A9_23(p1,T,f,boxsize) + A10_23(p1,T,f,boxsize) + A11_23(p1,T,f,boxsize)
...
def R12R24(p1,T,f,boxsize):
    coefficient = 1/(2**4*(2*np.pi)**3*p1**2)
    integral12 = A1_12(p1,T,f,boxsize) + A2_12(p1,T,f,boxsize) + A3_12(p1,T,f,boxsize) + A4_12(p1,T,f,boxsize) + A5_12(p1,T,f,boxsize) + A6_12(p1,T,f,boxsize)
    integral24 = A1_24(p1,T,f,boxsize) + A2_24(p1,T,f,boxsize) + A3_24(p1,T,f,boxsize) + A4_24(p1,T,f,boxsize) + A5_24(p1,T,f,boxsize) + A6_24(p1,T,f,boxsize) + A7_24(p1,T,f,boxsize) + A8_24(p1,T,f,boxsize) + A9_24(p1,T,f,boxsize) + A10_24(p1,T,f,boxsize)
...
@nb.jit(nopython=True, parallel=True)
def driver_short(p_array,T,f,boxsize):
    boxsize = p_array[1]-p_array[0]
    output_array = np.zeros(len(p_array))
    for i in nb.prange (1,len(p_array)//2):
        #    for i in nb.prange (1,len(p_array)):
        if p_array[i]<.15791:
            output_array[i] = R11R21(p_array[i],T,f,boxsize)
        elif p_array[i]<.18067:
